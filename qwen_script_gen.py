...
if LLM=="qwen":
    logging.info("Using QWEN")
    from arc import qwen_generate as cg
...
def create_arc(arc_data: dict) -> Arc:
    return Arc(
        type=arc_data.get("type", "无类型"),
        description=arc_data.get("description", "无描述")
    )
...
def create_characters(characters_data: list) -> list:
    characters = []
    for index, char_data in enumerate(characters_data):
        # 输出调试信息，检查角色数据
        print(f"正在处理第 {index + 1} 个角色的数据：", char_data)

        # 获取 personality_traits 数据并创建 PersonalityTraits 对象
        ...
        # 获取 arc 数据并创建 Arc 对象
        arc_data = char_data.get("arc", {})
        
        arc = create_arc(arc_data)
        ...
        character = Character(
            ...
            arc=arc  # 使用解析后的 arc 对象
        )

        # 输出 arc 字段的调试信息
        print(f"第 {index + 1} 个角色的 arc: {character.arc}")
...
def actor_agent(modified_script_elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    原始对白：
    {element['content']}
    请对上述对白进行优化，输出格式与原始对白一致，严格以 JSON 格式返回，不添加任何解释：
    {{
        "dialogue": "[角色姓名]: （动作描述）对白"
    }}
    """
...
- 旁白应尽量少，只有在对白无法表达重要信息时才添加。
- 旁白应简洁明了，帮助观众理解情节或角色内心。
- **不要删除或修改原有对白**。请以以下 JSON 格式返回，不添加任何解释：
        {{
            "narration": "旁白内容"
        }}
...
def scriptwriter_agent(
    ...
    n_turning_points: int = 1, 
    # act_number=1, # 生成n个转折
    is_final_scene: bool = False ,
     theme='' # 添加这个参数来区分是否是最后一个场景
) -> Dict[str, Any]:
    """
    ...
    prompt_turning_scene=f"主题是{theme}，"
    
    if is_final_scene:
        prompt_turning_scene += f"""
        你是一名出色的舞台剧作家。基于以下剧情大纲和角色信息，**这是最后一幕的最后一个情境，生成策略是冲突解决、角色和解、未来展望。**
        请生成一个**详细、具体**的情境描述，作为故事的收尾。**不要添加转折**，而是要自然地让故事走向结局，解开之前的悬念并解决冲突。这个情境描述应包含充满张力的对话和行动，同时避免过于突兀或仓促的结尾。要给故事一个大结局。所有人物都有交代。
        
        情境信息: {scene.description}
        环境信息: {scene.environment['description']}
        注意：
        **如果这个情境信息不是十分详细，分别是情境信息里省略的部分，如“经过一系列事件/斗争/阻碍”，还有一笔带过不易生成对白的地方，你都应该给它补充非常细致的细节。情境里如果包含伏笔和线索，一定要在你生成的新版本里被凸显。补充的细节要曲折，新奇，有趣，自然，但不平庸。**
        """
    ...
    # 步骤一：生成含转折的情境描述
        ...
        如果是第一幕，角色信息应该通过合适的对白被自然而然交代清楚给观众。
...
# 基于以下场景描述（第 {i+1} 段）为该段生成**至少100句**对白和动作，表现生动有趣的情节。对白要能通过语言来突出**具体的行动细节**、和事件、冲突、不可逆决策。确保每个角色的台词体现其性格、目标、情感状态和与其他角色的关系。对白应合理、成熟，符合角色的背景和当前情境。
...
# """
        prompt_key_dialogues = f"""
        你是一个优秀的舞台剧作家。这是剧本的第{scene.act_number}幕，故事描述（第 {i+1} 段）:
        {segment}
        基于以上故事描述为该段生成**100句以上的**对白和动作，**所有的情节都要通过你生成的对白得到细致的表达**。对白要能通过语言来突出**具体的行动细节**、和事件、冲突、不可逆决策。确保每个角色的台词体现其性格、目标、行动、弧光。对白应合理、成熟，符合角色的背景和当前情境。
        **对白应符合它在整个剧本中所处位置所决定的策略：{strategy}**
        *对白不是直接表达角色的心理活动或是感慨等。对白都是为了叙事而服务。*也就是通过谈话，把事件准确，完整地刻画出来。如果你觉得事件不够详细，可以先补充事件的细节，但是不用生成出来，而是通过对白来表现出来。
        **如果是第一幕，角色信息应该通过合适的对白被自然而然交代清楚给观众。**
        **如果是第四幕，对白应该把大结局展示的完整，清楚。**
        参考阿加莎克里斯蒂，柯南道尔，东野圭吾等作家的对白风格。参考曹禺、老舍的文学风格。
        情节中要巧妙地融入以下元素：
        1. **悬疑和诡计**：通过对话和动作暗示隐藏的秘密或诡计，增加剧情的深度和复杂性。
        2. **冲突和紧张**：对话中应包含角色间的冲突、紧张关系和情感波动，推动剧情发展。
        3. **动作和互动**：结合对白生成角色的具体动作和互动，使场景更加生动和具象。
        4. **子文本和暗示**：通过隐含的信息和暗示，使对白具有多层次的解读空间，增强观众的参与感和思考。
        5. **巧妙的诡计**：在对话中隐含巧妙的诡计，使剧情发展出乎意料但逻辑自洽。
...
def testmain(plot_outline, scenes, enable_black_humor=False, enable_metaphor=True, metaphor_probability=0.2, top_k_style=5, script_title='test', author='309', filename='',theme=''):
    # 初始化 FAISS 索引
    faiss_metaphor_index, metaphor_documents, faiss_style_index, style_documents = initialize_indices()
    ...
    f= open(f"{filename}_all_scenes.json", "w")
    f.writelines(plots)
...
def generate_script(enable_black_humor=False, enable_metaphor=False, metaphor_probability=0.2, top_k_style=5,filename='generated_script',theme='反英雄',fileprefix='great25.2add'):
    

    # 读取 plot_outline.json
    # fileprefix='great25.2add'
    try:
    ...
    testmain(plot_outline, scenes, enable_black_humor=True, enable_metaphor=True, metaphor_probability=0.2, top_k_style=5,filename=fileprefix+'_script.json',theme='反英雄')
import concurrent.futures
def run_generate_scripts_concurrently(tasks: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    """
    并发运行多个 generate_script 调用。
    
    参数:
        tasks: 包含每个任务的参数字典列表，每个字典应包含 'fileprefix' 和 'theme'。
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(tasks)) as executor:
        # 提交所有任务
        future_to_task = {executor.submit(generate_script, task['fileprefix'], task['theme']): task for task in tasks}
        ...
        for future in concurrent.futures.as_completed(future_to_task):
            ...
            try:
                ...
                logging.info(f"任务成功: fileprefix={result['fileprefix']}, theme={result['theme']}")
                results.append(result)
            except Exception as exc:
                logging.error(f"任务失败: fileprefix={task['fileprefix']}, theme={task['theme']}, 错误={exc}")
    return results
if __name__ == "__main__":
    # generate_script(fileprefix='great25.2add',theme='反英雄')
    generate_script(fileprefix='great13add',theme='悬疑破案')
    generate_script(fileprefix='great12.8add',theme='复仇')
    # tasks = [
    #     {'fileprefix': 'great25.2add', 'theme': '反英雄'},
    #     {'fileprefix': 'great13add', 'theme': '悬疑破案'},
    #     {'fileprefix': 'great12.8add', 'theme': '复仇'},
    # ]
    
    # # 运行并发任务
    # results = run_generate_scripts_concurrently(tasks)
    
    # # 输出结果
    # for result in results:
    #     print(f"生成剧本成功: 文件前缀={result['fileprefix']}, 主题={result['theme']}, 状态={result['status']}")