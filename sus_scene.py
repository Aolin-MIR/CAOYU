...
def generate_fate_reveal(event_characters, full_event_description):
    ...
    return fate_reveal
def create_cao_yu_style_scene(characters, graph, history, main_line=True, scene_number=1, environment=None, act_number=1, final_scene=False,act_goals=None,history_line=None):
    """
    ...
    f"这一幕的目标是：{act_goal}。生成一个强调阶段性目标的情境，并融合主副线角色。"
    f"生成一个情感强烈、具有命运感的情境，强调角色之间的冲突与不可控的命运力量。"
    f"注重情感细腻的描绘，情感表达应当克制而深刻，避免直白的对抗与感情宣泄。\n")
...
def dynamic_scene_generation(input_data,theme):
    ...
    while act_counter <= 3:  # 假设4幕结构
        ...
        act_completed = False  # 标志当前幕是否完成
        # if act_counter>2: max_scenes_per_act=1

        if final_merge :
            # 主副线融合或交织场景
            scene = create_cao_yu_style_scene(
        ...
        if final_merge:
            ...
            if goal_check.get('合格与否', 0) == 1:
                ...
                logging.info(f"第 {act_counter} 幕生成了 {max_scenes_per_act} 个场景，目标未达成，重新生成本幕。原因: {goal_check.get('原因', '无原因说明')}")
                # 回滚人物、环境、历史和场景
        ...
        if act_counter == 4:
            ...
            if not has_multiple_interpretations:
                print("结局没有多重解读，重新生成最后一幕。")
                act_counter -= 1  # 回到上一幕，重新生成
            else:
                print("多重解读存在，继续进入下一幕。")

        act_counter += 1
    is_valid=False
    reasons=None
    environment_description = (
        f"环境描述：{environment.change_environment( history, act_goals[4], main_line=True,history_line=None)}\n"
...
if __name__ == "__main__":
    ...
    if os.path.exists('great13.json'):
        scenes_generated = load_scenes_from_json('great13.json')
    else:
        scenes_generated = dynamic_scene_generation(input_data100,'悬疑')
        save_scenes_to_json(scenes_generated, 'great13.json')
    scenes_generated = add_foreshadowing_and_clues_to_scene_list(scenes_generated,'悬疑')
    ...
    # 保存生成的场景到 JSON 文件
    save_scenes_to_json(scenes_generated, 'great13add.json')